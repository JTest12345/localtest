...
namespace EICS
{
    public partial class SettingInfo
    {
        ...
        /// <param name="plantCD"></param>
        /// <returns></returns>
        public bool IsMFileExists(string plantCD)
        {
            EquipmentInfo equipmentInfo = EquipmentList.Find(e => e.EquipmentNO == plantCD);

            return equipmentInfo.MFileExists;
        }
		...
		private void LoadSettingPerLine(XmlDocument doc)
		{
			for (int lineCount = 0; lineCount < doc.SelectNodes(LINEINFO_NODE).Count; lineCount++)
			{
				...
				settingInfoPerLine.EquipmentList = GetEquipSetting(doc, lineCount, settingInfoPerLine.LineCD);
		...
		/// <returns></returns>
		private List<EquipmentInfo> GetEquipSetting(XmlDocument doc, int lineCount, int lineCD)
		{
				...
				foreach (int lineCd in lineList)
				{
					...
					foreach (Lset lsetData in lsetDataList)
					{
						...
						for (int i = 0; settingInfo.SettingInfoList.Count() > i; i++)
						{
                            ...
                            if (equipmentInfo != null)
                            {
                                // TODO 吉本さんに下記インスタンスのプロパティを変える事で設定ファイルのインスタンスの内容が変わるか聞く
