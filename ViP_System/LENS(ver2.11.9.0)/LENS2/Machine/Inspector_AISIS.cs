...
namespace LENS2.Machine
{
    ...
    /// 外観検査機
    /// </summary>
    public class Inspector_AISIS : MachineBase, IMachine
    {
        ...
        public string FullInspectionFlagAddress { get; set; }
        ...
        public int MMFileResultCodeStartColumn { get; set; }

        public int MMFileAddressStartColumn { get; set; }

        public int MMFileKeikouColumn { get; set; }

        /// <summary>
        /// カメラの数(MMファイル内の検査結果を何列分参照するか)
        /// </summary>
        ...
        public void MainWork()
        {
            try
            {
                ...
                if (isRequestTypeCD() && isOperationalStatus())
                {
                    ...
                    //if (isFullInspectionNeedToSend())
                    //{
                    //    // 全数検査フラグ送信処理
                    //    fullInspectionFlagSendProcess(lot.IsFullSizeInspection);
                    //}
                    //else
                    //{
                        // マッピングデータ送信処理
        ...
        private void fullInspectionFlagSendProcess(bool isFullInspection)
        {
            Plc.SetBit(this.FullInspectionFlagAddress, isFullInspection);
        }
		...
		public class MMFile : MachineLog
		{
            ...
            public int CONTENTS_NEXT_RESULT_CT = 2;
			...
			private Dictionary<int, string> getInspectionResult(string[] fileValue, string magazineQR)
			{
                ...
                int addressColumn = ((Inspector_AISIS)this.WorkMachine).MMFileAddressStartColumn;
                int resultColumn = ((Inspector_AISIS)this.WorkMachine).MMFileResultCodeStartColumn;

                for (int j = 0; j < ((Inspector_AISIS)this.WorkMachine).CameraCount; j++)
                {
                    for (int i = 0; i < fileValue.Count(); i++)
                    {
                        if (string.IsNullOrWhiteSpace(fileValue[i]))
                        ...
                        if (fileLineValue[((Inspector_AISIS)this.WorkMachine).MMFileKeikouColumn].Trim() == CONTENTS_KEIKOUFG_OFF)
                        {
                            // 傾向管理しないデータの場合、次へ
                            continue;
                        }

                        int address = Convert.ToInt32(fileLineValue[addressColumn].Trim());
                        ...
                        string result = fileLineValue[resultColumn].Trim();
                        if (retv.Keys.Count(k => k == address) == 0)
                        {
                            retv.Add(address, result);
                        }
                        else
                        {
                    ...
                    // 次カメラ列に進ませる
                    addressColumn += CONTENTS_NEXT_RESULT_CT;
                    resultColumn += CONTENTS_NEXT_RESULT_CT;
			...
			public void Run(Dictionary<int, string> inspectionMacSetting, Lot lot, bool isEnableMarkingRequest)
			{
				...
				if (isEnableMarkingRequest && lot.IsMappingInspection)
				{
        ...
        ///// </summary>
        ///// <returns></returns>
        //private bool isFullInspectionNeedToSend()
        //{
        //    string modelNm = LENS2_Api.MachineInfo.GetEicsModel(this.NascaPlantCD);
        //    if (string.IsNullOrEmpty(modelNm))
        //    {
        //        throw new ApplicationException(string.Format("EICS装置マスタに{0}が存在しない為、全数検査フラグを装置に送信する判断ができません。", this.NascaPlantCD));
        //    }

        //    if (modelNm == "Inspector_AISIS")
        //    {
        //        return true;
        //    }
        //    else
