...
namespace postEpoxyCopy
{
    class Program
    {
        ...
        public void CopyTest(string ffrom)
        {
            ...
            string ip = ffrom.Substring(2, ffrom.Length - 5);

            //string q = @"\\192.168.4.86\log";
            //q = q.Substring(2, q.Length - 5);
            //ip = "";

            //testpass
            //ip = @"192.168.4.92\";
            string folderTo = @"\\svfile7\境川\工程データ\postEpoxy\" + ip + yyyymmdd;
            string filePattern = "post*Epoxy*";
            ...
            sf.wFunc = NativeMethods.FileFuncFlags.FO_COPY; //移動を指示します。
            sf.fFlags = NativeMethods.FILEOP_FLAGS.FOF_RENAMEONCOLLISION;//同じ名前のファイル名があった場合、末尾にコピーをつけて保存
