...
BOOL CElectBalance_JNverDlg::OnInitDialog()
{
	...
	if( CreateDirectory(_T("C:\\EB\\pcs"), NULL )){}

	if( CreateDirectory(_T("C:\\EB\\mst"), NULL )){}

	if( CreateDirectory(_T("C:\\EB\\Backup"), NULL )){}
	...
	if(fp.Open(ini, CFile::modeWrite | CFile::shareDenyNone)){
				...
				fp.WriteString(_T("\n"));
				fp.WriteString(_T("④樹脂の設定値修正履歴ファイルのパス"));
				fp.WriteString(_T("\n"));
				fp.WriteString(_T("⑤計量時の更新頻度(ms)"));
				fp.WriteString(_T("\n"));
				fp.WriteString(_T("⑥誤差範囲(%)"));
				fp.WriteString(_T("\n"));
...
BOOL bRet = CopyFile(_T("C:\\EB\\mst\\Jyushidata.csv"), _T("C:\\EB\\Backup\\Jyushidata.csv"), FALSE);

CString Bf;
CString outname = _T("C:\\EB\\Backup\\Jyushidata_backup_")+ date + _T(".zip");
CString fname = _T("C:\\EB\\mst\\Jyushidata.csv");

	Bf.Format(_T("-m -q \"%s\" \"%s\""),outname, fname);//zip

	int res = 0;
	//res = Unlha(NULL, buf, NULL, NULL);//lzh

	char TEMP[512];
	wcstombs(TEMP, Bf, 512);
	res = Zip(NULL, TEMP, NULL, NULL);//zip


MoveFile(_T("C:\\EB\\Backup\\Jyushidata.csv"), _T("C:\\EB\\mst\\Jyushidata.csv"));
