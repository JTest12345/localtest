...
void CRenum::OnBnClickedButtonExe()
{
	...
	Newg.Format(_T("%.4f"), ReNum);//修正後の値(比率変える前)
	pWnd->Jshg.msr.ListC[pWnd->Jshg.msr.CheckBoxNum][2] = Newg;//リストコントロールは比率を変える前の値を代入
	ReNum = ReNum / pWnd->Jshg.msr.Num;//比率を変える

	//pWnd->Jshg.msr.formdata = _tcstod(Edit_V_rn,NULL);//入力値を保存しておく

	Oldg = pWnd->Jyushi_List[i][(3*Num)+5];//修正前の値
	Newg.Format(_T("%.4f"), ReNum);//修正後の値
