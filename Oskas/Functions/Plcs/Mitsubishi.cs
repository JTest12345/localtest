...
namespace Oskas.Functions.Plcs
{
	...
	public class Mitsubishi : IDisposable, IPLC
	{
        ...
        /// <param name="timeout">タイムアウトまでの秒数（sec) 0または負なら無限ループ</param>
        /// <param name="exitValue">監視を抜ける条件</param>
        /// <param name="act">毎ループ時に実行する処理または処理関数</param>
        /// <returns>タイムアウト時false</returns>
        public bool WatchBit(string hexAddressWithDeviceNM, int timeout, string exitValue, Action act)
        {
			...
			while (true)
			{
				...
				{
					//Log.RBLog.Error("WatchBit PLC通信エラー発生　1秒待機後に監視継続:" + ex.ToString());
		...
		/// <summary>
		/// PLCのデバイスアドレス表記が16進表記を定められたデバイスかどうかを判定する
		/// </summary>
		/// <param name="plcMemAddr"></param>
		/// <returns></returns>
		public static bool isHexFormAddress(string plcMemAddr)
		{
            //(MELSECコミュニケーションプロトコルリファレンスマニュアル(201406).pdf P.66、67 デバイスコード一覧を参照して記述)

            string plcMemForwardPart = plcMemAddr.Substring(0, 2);

            switch (plcMemForwardPart)
            {
                case "FX":
                    return true;
                case "FY":
            ...
            plcMemForwardPart = plcMemAddr.Substring(0, 1);
		...
		private static int getDeviceKindLen(string plcMemAddr)
		{
			//(MELSECコミュニケーションプロトコルリファレンスマニュアル(201406).pdf P.66、67 デバイスコード一覧を参照して記述)
			string plcMemForwardPart = plcMemAddr.Substring(0, 2);
			...
			switch (plcMemForwardPart)
			{
				...
				case "G":
					return 1;
				default:
					throw new ApplicationException("PLCメモリアドレスの文字列からデバイス種別部の文字数を取得しようとしましたがデバイス種別が想定していない文字です。メモリアドレス：" + plcMemAddr);
			}
		}

		/// <summary>
		/// 引数で渡したplcMemAddrにaddValueを加算した後のアドレスを取得する
		/// </summary>
		/// <param name="plcMemAddr">デバイス種別を含むアドレス　ビット指定の表記を含むアドレスには非対応</param>
		/// <param name="addValue">指定したplcMemAddrへ加算する値</param>
		/// <returns></returns>
		public static string GetMemAddrAfterAdding(string plcMemAddr, int addValue)
		{
			int devKindLen = getDeviceKindLen(plcMemAddr);
			string plcMemNoPart = plcMemAddr.Substring(devKindLen, plcMemAddr.Length - devKindLen);
            string retv;
            
			int plcMemNo;

			if (isHexFormAddress(plcMemAddr))
			{
				plcMemNo = Convert.ToInt32(plcMemNoPart, 16);
			}
			else
			{
				if(int.TryParse(plcMemNoPart, out plcMemNo) == false)
				{
					string errMsg = $"PLCメモリアドレスの数値部分に加算を加える処理で、数値該当部分の文字列の数値変換に失敗しました。メモリ:{plcMemAddr}/数値と判断した部分:{plcMemNoPart}";
					throw new ApplicationException(errMsg);
				}				
			}

			int addedPlcMemNo = plcMemNo + addValue;

            if (isHexFormAddress(plcMemAddr))
            {
                retv = plcMemAddr.Substring(0, devKindLen) + addedPlcMemNo.ToString("X6"); //MELSEC Qはアドレスの数値部分は6桁固定
            }
            else
            {
                retv = plcMemAddr.Substring(0, devKindLen) + addedPlcMemNo.ToString("D6");
            }
		...
		public static bool ExchangeDecAddressDevtoHex(string devaddress, ref string hexaddress)
		{
			try
			{
				var intaddress = int.Parse(devaddress.Substring(1).PadLeft(6, '0'));
				var header = devaddress[0].ToString();
				hexaddress = header + intaddress.ToString("x6");
				return true;
			}
			catch (Exception)
			{
