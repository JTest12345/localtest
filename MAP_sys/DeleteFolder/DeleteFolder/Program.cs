...
namespace VIP_DBDataCopy
{
    class Program
    {
        ...
        /// </summary>
        public static string DeleteFolderPath1 { get; set; }
        public static string DeleteFolderPath2 { get; set; }
        public static string DeleteFolderPath3 { get; set; }
        public static string DeleteFolderPath4 { get; set; }
        public static string DeleteFolderPath5 { get; set; }
        public static string DeleteDate { get; set; }
        ...
        static void Main(string[] args)
        {
            ...
            try
            {               
                DeleteFolderPath1 = ConfigurationManager.AppSettings["DeleteFolderPath1"];
                DeleteFolderPath2 = ConfigurationManager.AppSettings["DeleteFolderPath2"];
                DeleteFolderPath3 = ConfigurationManager.AppSettings["DeleteFolderPath3"];
                DeleteFolderPath4 = ConfigurationManager.AppSettings["DeleteFolderPath4"];
                DeleteFolderPath5 = ConfigurationManager.AppSettings["DeleteFolderPath5"];

                passlist = new[] { DeleteFolderPath1, DeleteFolderPath2, DeleteFolderPath3, DeleteFolderPath4, DeleteFolderPath5 };
                DeleteDate = ConfigurationManager.AppSettings["DeleteDate"];
            }
            catch (Exception ex)
            {
                return;
            ...
            foreach(string Deletepass in passlist)
            {
                obj.DeleteFile(Deletepass, DeleteDate);
            }

            /*
            if (DirectoryExists(DeleteFolderPath1))
            {
                obj.DeleteFile(DeleteFolderPath1,DeleteDate);
            }

            if (DirectoryExists(DeleteFolderPath2))
            {
                obj.DeleteFile(DeleteFolderPath2, DeleteDate);
            }

            if (DirectoryExists(DeleteFolderPath3))
            {
                obj.DeleteFile(DeleteFolderPath3, DeleteDate);
            }

            if (DirectoryExists(DeleteFolderPath4))
            {
                obj.DeleteFile(DeleteFolderPath4, DeleteDate);
            }

            if (DirectoryExists(DeleteFolderPath5))
            {
                obj.DeleteFile(DeleteFolderPath5, DeleteDate);
            }
            */
        }

        public void DeleteFile(string DeleteFolderPath, string DeleteDate)
        {
            string folderFrom = DeleteFolderPath; //コピー元のフォルダー
            ...
            DateTime filetime;
            int dd = int.Parse(DeleteDate) * -1;
            DateTime d = DateTime.Now.AddDays(dd).Date;


            //サブフォルダの中身含めてを削除する処理(フォルダは削除しない)
            string[] deletefiles = System.IO.Directory.GetFiles(folderFrom, "*", System.IO.SearchOption.AllDirectories);

            foreach (string filepass in deletefiles)
            {
                filetime = File.GetLastWriteTime(filepass);
                if (filetime.Date <= d)
                {
            ...
            //指定のフォルダのみを削除する処理
            //DirectoryInfo directory = new DirectoryInfo(folderFrom);
