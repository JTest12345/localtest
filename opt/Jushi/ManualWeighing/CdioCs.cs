...
/*
 * CONTECからダウンロードしたAPI-TOOL(WDM)を使う
 * 最初にダウンロードしたzipを解凍してAPIPAC\DioWdm\Disk1\setup.exeを実行する
 * そこで指定したインストール先を探すと、c#サンプルの中にCdioCs.csファイルがある
 * 
 * クラス継承して使うことにする
 */
namespace ManualWeighing {
    ...
    /// CONTECデバイスを扱うクラス
    /// </summary>
    public class ContecDio : CdioCs.Cdio {
        ...
        /// コンストラクタ
        /// </summary>
        public ContecDio(string device_name) {
        ...
        /// CONTECデバイスの初期化処理
        /// </summary>
        /// <exception cref="失敗した時は例外発生"></exception>
        public void Initialize() {
            int ret = Init(DeviceName, out dio_id);

            if (ret != 0) {
                string error_msg = "";
                GetErrorString(ret, out error_msg);

                throw new Exception($"CONTECデバイスの初期化処理に失敗しました。\n\n{error_msg}");
            }
        ...
        /// 指定した入力ポートのビットを読み取る
        /// </summary>
        /// <param name="input_bit"></param>
        /// <returns></returns>
        /// <exception cref="読み取り失敗した時は例外発生"></exception>
        public byte Get_InputBit(short input_bit) {

            byte data;

            //入力ビットの読み取り
            int ret = InpBit(dio_id, input_bit, out data);

            string error_msg = "";
            if (ret != 0) {
                GetErrorString(ret, out error_msg);
                throw new Exception($"入力ビットの読み取りに失敗しました。\n\n{error_msg}");
            }
            else {
                return data;
            }
