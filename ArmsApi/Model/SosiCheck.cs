...
namespace ArmsApi.Model
{
    class SosiCheck
    {
        ...
        /// <summary>
        /// TnTrn情報から使用した素子の波長ランクを取得する
        /// </summary>
        /// <param name="macno"></param>
        /// <param name="startdt"></param>
        /// <param name="enddt"></param>
        /// <returns></returns>
        public static string GetHchRnkFromOrder(long macno, DateTime startdt, DateTime? enddt)
        {
            string retv = "";
            ...
            try
            {
                ...
                using (SqlCommand cmd = con.CreateCommand())
                {
                    ...
                    cmd.CommandText = @"
                        SELECT DISTINCT MT.blendcd
                        FROM TnMacMat M
                        INNER JOIN TnMaterials MT
                        ON M.materialcd = MT.materialcd
                        AND m.lotno = MT.lotno
                        WHERE M.macno = @MACNO
                        AND(M.enddt IS Null OR M.enddt >= @STARTDT)
                        AND(M.startdt IS Null OR M.startdt <= @ENDDT)
                        AND MT.delfg = 0
                        AND MT.iswafer = 1
                        ";
                    ...
                    cmd.Parameters.Add("@ENDDT", System.Data.SqlDbType.DateTime).Value = enddt;

                    using (SqlDataReader rd = cmd.ExecuteReader())
                    {
                        List<string> ircdlst = new List<string>();
                        while (rd.Read())
                        {
                            if (SQLite.ParseString(rd["blendcd"]).Length > 2)
                            {
                                ircdlst.Add(SQLite.ParseString(rd["blendcd"]).Substring(2));
                            }
                        }
                        if (ircdlst.Count() > 0)
                        {
                            List<HchRnkCejKsh> rnklst = SosiCheck.GetHchRnkCejKshList(ircdlst);
                            if (rnklst.Count() > 0)
                                retv = rnklst[0].CejHchRnk;
                        }
                    }
                }
            }
