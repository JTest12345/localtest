...
namespace ARMS3.Model.PLC
{
	...
	public class Omron : IDisposable, IPLC
	{
		...
		private byte[] executeCommand(byte[] bynaryCommand, bool canRetry)
		{
            ...
            System.Threading.Mutex mutex = new System.Threading.Mutex(false, $"OmronPlcAccessMutex_{IPAddress.Trim()}");

            try
            {
                //EICSとの同時通信を規制するためにMutexでロックする。1秒待機しても解除されなかったら例外エラー。
                ...
                {
                    throw new ApplicationException($"OmronPLCへのデータ送信で他プロセスのロックが1秒以上解除されませんでした。IP:{IPAddress}");
                }
			}
			catch (Exception ex)
			{
